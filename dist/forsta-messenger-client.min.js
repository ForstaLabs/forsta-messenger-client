...
;if(s&&s.length)for(const t of s)try{await t.apply(e,e.data.args)}catch(e){console.error("ifrpc event listener error:",t,e)}else console.debug("ifrpc event triggered without listeners:",e.data.name)}}e.init=((e,s)=>new a(e,s))}(),self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{},function(){"use strict";forsta.messenger.Client=class e{constructor(e,s,t){if(!(e instanceof Element))throw new TypeError("el argument must be an Element");if(!s)throw new TypeError("auth argument missing");this.auth=s,this.options=t||{},this.callback=t.callback,this._iframe=document.createElement("iframe"),this._iframe.style.border="0 solid transparent",this._iframe.style.width="100%",this._iframe.style.height="100%",this._iframe.addEventListener("load",()=>{const e=this._iframe.contentWindow;this._rpc=ifrpc.init(this._iframe.contentWindow),this._rpc.addEventListener("init",this.onClientInit.bind(this))});const i=t.url||"https://app.forsta.io/@";this._iframe.setAttribute("src",`${i}?managed`),
e.appendChild(this._iframe)}async onClientInit(){console.debug("client init"),await this._rpc.invokeCommand("configure",{auth:this.auth,showNav:!!this.options.showNav,showHeader:!!this.options.showHeader,showThreadAside:!!this.options.showThreadAside,showThreadHeader:!!this.options.showThreadHeader}),this.callback&&this.callback(this)}}}();