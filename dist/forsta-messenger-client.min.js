...
;if(s&&s.length)for(const t of s)try{await t.apply(e,e.data.args)}catch(e){console.error("ifrpc event listener error:",t,e)}else console.debug("ifrpc event triggered without listeners:",e.data.name)}}e.init=((e,s)=>new a(e,s))}(),self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{},function(){"use strict";forsta.messenger.Client=class e{constructor(e,s,t){if(!(e instanceof Element))throw new TypeError("el argument must be an Element");if(!s)throw new TypeError("auth argument missing");this.auth=s,this.options=t||{},this.callback=this.options.callback,this._iframe=document.createElement("iframe"),this._iframe.style.border="none",this._iframe.style.width="100%",this._iframe.style.height="100%",this._iframe.setAttribute("allow","camera; microphone; fullscreen; autoplay; display-capture; geolocation; speaker; vibrate;"),this._iframe.setAttribute("allowfullscreen","true"),this._iframe.addEventListener("load",()=>{this._rpc=ifrpc.init(this._iframe.contentWindow),
this._rpc.addEventListener("init",this._onClientInit.bind(this))});const r=this.options.url||"https://app.forsta.io/@";this._iframe.setAttribute("src",`${r}?managed`),e.appendChild(this._iframe)}async _onClientInit(){if(await this._rpc.invokeCommand("configure",{auth:this.auth,showNav:!!this.options.showNav,showHeader:!!this.options.showHeader,showThreadAside:!!this.options.showThreadAside,showThreadHeader:!!this.options.showThreadHeader}),this._rpcEarlyEvents){for(const e of this._rpcEarlyEvents)this._rpc.addEventListener(e.event,e.callback);delete this._rpcEarlyEvents}this.callback&&await this.callback(this)}addEventListener(e,s){this._rpc?this._rpc.addEventListener(e,s):(this._rpcEarlyEvents||(this._rpcEarlyEvents=[]),this._rpcEarlyEvents.push({event:e,callback:s}))}removeEventListener(e,s){this._rpc?this._rpc.removeEventListener(e,s):this._rpcEarlyEvents=this._rpcEarlyEvents.filter(t=>!(t.event===e&&t.callback===s))}async navPanelToggle(e){
