...
;forsta.messenger.Client=class t{constructor(t,r,n){if(!(t instanceof Element))throw new TypeError("el argument must be an Element");if(!r)throw new TypeError("auth argument missing");this.auth=r,this.options=n||{},this.onInit=this.options.onInit,this.onLoaded=this.options.onLoaded,this._iframe=document.createElement("iframe"),this._iframe.style.border="none",this._iframe.style.width="100%",this._iframe.style.height="100%";const s=new Set(["camera","microphone","fullscreen","autoplay","display-capture","geolocation","speaker","vibrate"]);if(document.featurePolicy&&document.featurePolicy.allowedFeatures){const e=new Set(document.featurePolicy.allowedFeatures());for(const t of Array.from(s))e.has(t)||s.delete(t)}this._iframe.setAttribute("allow",Array.from(s).join("; ")),s.has("fullscreen")&&this._iframe.setAttribute("allowfullscreen","true");const o=this.options.url||"https://app.forsta.io/@";this._iframe.src=`${o}?managed`,t.appendChild(this._iframe),
this._iframe.contentWindow.addEventListener("beforeunload",e=>{console.error("before unload")}),this._iframe.contentWindow.addEventListener("unload",e=>{console.error("unload")}),this._iframe.contentWindow.addEventListener("unload",e=>{console.error("unload")}),this._iframe.contentWindow.addEventListener("load",e=>{console.error("load",this._iframe.src,this._iframe.getAttribute("src"))}),this._iframe.addEventListener("load",e=>{console.error("load iframe",e),console.error("load iframe",this._iframe.src,this._iframe.getAttribute("src"))}),this._iframe.addEventListener("loadstart",e=>{console.error("loadstart iframe")}),this._iframe.addEventListener("loadend",e=>{console.error("loadend iframe")}),this._rpc=ifrpc.init(this._iframe.contentWindow,{acceptOpener:!0}),this._idbGateway=new e.IDBGateway(this._rpc);const i=this;this._rpc.addEventListener("init",function(e){const t=this;i._onClientInit(t.source,e)}),this.onLoaded&&this._rpc.addEventListener("loaded",()=>this.onLoaded(this))}
async _onClientInit(e,t){const r={auth:this.auth};if("main"===t.scope&&(Object.assign(r,{showNav:!!this.options.showNav,showHeader:!!this.options.showHeader,showThreadAside:!!this.options.showThreadAside,showThreadHeader:!!this.options.showThreadHeader,ephemeralUser:this.options.ephemeralUserInfo,openThreadId:this.options.openThreadId}),this._rpcEarlyEvents)){for(const e of this._rpcEarlyEvents)this._rpc.addEventListener(e.event,e.callback);delete this._rpcEarlyEvents}await this._rpc.invokeCommandWithFrame(e,"configure",r),"main"===t.scope&&this.onInit&&await this.onInit(this)}addEventListener(e,t){this._rpc?this._rpc.addEventListener(e,t):(this._rpcEarlyEvents||(this._rpcEarlyEvents=[]),this._rpcEarlyEvents.push({event:e,callback:t}))}removeEventListener(e,t){this._rpc?this._rpc.removeEventListener(e,t):this._rpcEarlyEvents=this._rpcEarlyEvents.filter(r=>!(r.event===e&&r.callback===t))}async navPanelToggle(e){await this._rpc.invokeCommand("nav-panel-toggle",e)}
async threadStartWithExpression(e){return await this._rpc.invokeCommand("thread-join",e)}async threadOpen(e){await this._rpc.invokeCommand("thread-open",e)}async threadSetExpiration(e,t){await this._rpc.invokeCommand("thread-set-expiration",e,t)}async threadList(){return await this._rpc.invokeCommand("thread-list")}async threadListAttributes(e){return await this._rpc.invokeCommand("thread-list-attributes",e)}async threadGetAttribute(e,t){return await this._rpc.invokeCommand("thread-get-attribute",e,t)}async threadSetAttribute(e,t,r){return await this._rpc.invokeCommand("thread-set-attribute",e,t,r)}}}(),function(){"use strict";self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{};const e={User:{migrations:[{version:1,migrate:function(e){const t=e.db.createObjectStore("messages");t.createIndex("threadId-received",["threadId","received"]),t.createIndex("threadId-read",["threadId","read"]),t.createIndex("sent","sent"),t.createIndex("expire","expire")
;const r=e.db.createObjectStore("receipts");r.createIndex("messageId","messageId");const n=e.db.createObjectStore("threads");n.createIndex("type-timestamp",["type","timestamp"]),e.db.createObjectStore("sessions"),e.db.createObjectStore("identityKeys"),e.db.createObjectStore("preKeys"),e.db.createObjectStore("signedPreKeys"),e.db.createObjectStore("state")}},{version:2,migrate:function(e){const t=e.objectStore("threads");t.createIndex("timestamp",["timestamp"])}},{version:3,migrate:function(e){const t=e.db.createObjectStore("cache");t.createIndex("bucket-expiration",["bucket","expiration"])}},{version:4,migrate:function(e){}},{version:5,migrate:function(e){}},{version:6,migrate:function(e){e.db.createObjectStore("contacts")}},{version:7,migrate:function(e){const t=e.objectStore("messages");t.createIndex("member","members",{multiEntry:!0})}},{version:8,migrate:function(e){const t=e.objectStore("threads");t.createIndex("pendingMember","pendingMembers",{multiEntry:!0})}},{version:9,
migrate:function(e){}},{version:10,migrate:function(e){const t=e.objectStore("messages");t.createIndex("ngrams3","ngrams3",{multiEntry:!0})}},{version:11,migrate:function(e){const t=e.db.createObjectStore("protocolReceipts");t.createIndex("sent","sent")}},{version:12,migrate:function(e){}},{version:13,migrate:function(e){}},{version:14,migrate:async function(e){const t=e.objectStore("threads");t.createIndex("archived-timestamp",["archived","timestamp"]),t.deleteIndex("type-timestamp");const r=t.openCursor();await new Promise((e,t)=>{r.onsuccess=(r=>{const n=r.target.result;if(n)try{void 0===n.value.archived&&n.update(Object.assign(n.value,{archived:0})),n.continue()}catch(e){throw t(e),e}else e()}),r.onerror=(e=>{console.error("cursor error event:",e),t(new Error("cursor error"))})})}},{version:15,migrate:function(e){const t=e.objectStore("messages");t.deleteIndex("ngrams3"),t.createIndex("from-ngrams","_from_ngrams",{multiEntry:!0}),t.createIndex("to-ngrams","_to_ngrams",{
multiEntry:!0}),t.createIndex("body-ngrams","_body_ngrams",{multiEntry:!0})}},{version:16,migrate:function(e){e.db.createObjectStore("trustedIdentities")}},{version:17,migrate:function(e){const t=e.db.createObjectStore("quarantinedMessages");t.createIndex("source","source")}},{version:18,migrate:function(e){const t=e.db.createObjectStore("counters");t.createIndex("model-fk-slot",["model","fk","slot"])}},{version:19,migrate:function(e){const t=e.objectStore("counters");t.createIndex("model-fk-slot-key",["model","fk","slot","key"])}},{version:20,migrate:function(e){const t=e.objectStore("messages");t.createIndex("threadId-serverReceived",["threadId","serverReceived"]),t.deleteIndex("threadId-received")}},{version:21,migrate:function(e){const t=e.objectStore("messages");t.createIndex("messageRef","messageRef")}},{version:22,migrate:async function(e){const t=e.objectStore("messages");t.createIndex("threadId-timestamp",["threadId","timestamp"]),t.deleteIndex("threadId-serverReceived")
