...
this._rpc.addEventListener("init",this._onClientInit.bind(this))});const r=this.options.url||"https://app.forsta.io/@";this._iframe.setAttribute("src",`${r}?managed`),e.appendChild(this._iframe)}async _onClientInit(){if(await this._rpc.invokeCommand("configure",{auth:this.auth,showNav:!!this.options.showNav,showHeader:!!this.options.showHeader,showThreadAside:!!this.options.showThreadAside,showThreadHeader:!!this.options.showThreadHeader,ephemeralUser:this.options.ephemeralUserInfo}),this._rpcEarlyEvents){for(const e of this._rpcEarlyEvents)this._rpc.addEventListener(e.event,e.callback);delete this._rpcEarlyEvents}this.callback&&await this.callback(this)}addEventListener(e,s){this._rpc?this._rpc.addEventListener(e,s):(this._rpcEarlyEvents||(this._rpcEarlyEvents=[]),this._rpcEarlyEvents.push({event:e,callback:s}))}removeEventListener(e,s){this._rpc?this._rpc.removeEventListener(e,s):this._rpcEarlyEvents=this._rpcEarlyEvents.filter(t=>!(t.event===e&&t.callback===s))}
async navPanelToggle(e){return await this._rpc.invokeCommand("nav-panel-toggle",e)}async threadStartWithExpression(e){return await this._rpc.invokeCommand("thread-join",e)}async threadOpen(e){return await this._rpc.invokeCommand("thread-open",e)}}}();