...
;if(t&&t.length)for(const r of t)try{await r.apply(e,e.data.args)}catch(e){console.error("ifrpc event listener error:",r,e)}else console.debug("ifrpc event triggered without listeners:",e.data.name)}}e.init=((e,t)=>new s(e,t))}(),self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{},function(){"use strict";const e=forsta.messenger;forsta.messenger.Client=class t{constructor(t,r,n){if(!(t instanceof Element))throw new TypeError("el argument must be an Element");if(!r)throw new TypeError("auth argument missing");this.auth=r,this.options=n||{},this.onInit=this.options.onInit,this.onLoaded=this.options.onLoaded,this._iframe=document.createElement("iframe"),this._iframe.style.border="none",this._iframe.style.width="100%",this._iframe.style.height="100%";const o=new Set(["camera","microphone","fullscreen","autoplay","display-capture","geolocation","speaker","vibrate"]);if(document.featurePolicy&&document.featurePolicy.allowedFeatures){
const e=new Set(document.featurePolicy.allowedFeatures());for(const t of Array.from(o))e.has(t)||o.delete(t)}this._iframe.setAttribute("allow",Array.from(o).join("; ")),o.has("fullscreen")&&this._iframe.setAttribute("allowfullscreen","true");const s=this.options.url||"https://app.forsta.io/@";this._iframe.setAttribute("src",`${s}?managed`),t.appendChild(this._iframe),this._iframe.contentWindow.addEventListener("beforeunload",e=>{console.error("before unload")}),this._iframe.contentWindow.addEventListener("unload",e=>{console.error("unload")}),this._iframe.contentWindow.addEventListener("unload",e=>{console.error("unload")}),this._iframe.contentWindow.addEventListener("load",e=>{console.error("load")}),this._iframe.addEventListener("load",e=>{console.error("load iframe",e)}),this._iframe.addEventListener("loadstart",e=>{console.error("loadstart iframe")}),this._iframe.addEventListener("loadend",e=>{console.error("loadend iframe")}),this._rpc=ifrpc.init(this._iframe.contentWindow),
this._idbGateway=new e.IDBGateway(this._rpc),this._rpc.addEventListener("init",this._onClientInit.bind(this)),this.onLoaded&&this._rpc.addEventListener("loaded",()=>this.onLoaded(this))}async _onClientInit(){if(await this._rpc.invokeCommand("configure",{auth:this.auth,showNav:!!this.options.showNav,showHeader:!!this.options.showHeader,showThreadAside:!!this.options.showThreadAside,showThreadHeader:!!this.options.showThreadHeader,ephemeralUser:this.options.ephemeralUserInfo,openThreadId:this.options.openThreadId}),this._rpcEarlyEvents){for(const e of this._rpcEarlyEvents)this._rpc.addEventListener(e.event,e.callback);delete this._rpcEarlyEvents}this.onInit&&await this.onInit(this)}addEventListener(e,t){this._rpc?this._rpc.addEventListener(e,t):(this._rpcEarlyEvents||(this._rpcEarlyEvents=[]),this._rpcEarlyEvents.push({event:e,callback:t}))}removeEventListener(e,t){
this._rpc?this._rpc.removeEventListener(e,t):this._rpcEarlyEvents=this._rpcEarlyEvents.filter(r=>!(r.event===e&&r.callback===t))}async navPanelToggle(e){await this._rpc.invokeCommand("nav-panel-toggle",e)}async threadStartWithExpression(e){return await this._rpc.invokeCommand("thread-join",e)}async threadOpen(e){await this._rpc.invokeCommand("thread-open",e)}async threadSetExpiration(e,t){await this._rpc.invokeCommand("thread-set-expiration",e,t)}async threadList(){return await this._rpc.invokeCommand("thread-list")}async threadListAttributes(e){return await this._rpc.invokeCommand("thread-list-attributes",e)}async threadGetAttribute(e,t){return await this._rpc.invokeCommand("thread-get-attribute",e,t)}async threadSetAttribute(e,t,r){return await this._rpc.invokeCommand("thread-set-attribute",e,t,r)}}}(),function(){"use strict";self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{};const e={User:{migrations:[{version:1,migrate:function(e){
...
new Error(`Missing migrations for target version: ${r}`);for(const o of n)console.warn("DB migrating to:",o.version),await o.migrate(e),console.info("DB migrated successfully:",o.version)}async createHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName);return e.idFallback&&!r.autoIncrement&&(e.json[e.idAttribute]=e.idFallback),await new Promise((n,o)=>{let s;return t.oncomplete=(e=>n()),t.onerror=(e=>o(e.target.error)),(s=r.keyPath?r.add(e.json):r.add(e.json,e.json[e.idAttribute])).onerror=(e=>o(e.target.error)),t.commit&&t.commit(),e.json})}async updateHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName);return await new Promise((n,o)=>{let s;t.oncomplete=(e=>n()),t.onerror=(e=>o(e.target.error)),(s=r.keyPath?r.put(e.json):r.put(e.json,e.json[e.idAttribute])).onerror=(e=>o(e.target.error)),t.commit&&t.commit()})}async readHandler(e){
const t=this.db.transaction([e.storeName],"readonly"),r=t.objectStore(e.storeName);let n;if(e.json[e.idAttribute])n=r.get(e.json[e.idAttribute]);else if(e.index){const t=r.index(e.index.name);n=t.get(e.index.value)}else{let t=0;for(const o of r.indexNames){const s=r.index(o);if("string"==typeof s.keyPath&&1>t)void 0!==e.json[s.keyPath]&&(n=s.get(e.json[s.keyPath]),t=1);else if("object"==typeof s.keyPath&&s.keyPath.length>t){let r=!0;const o=s.keyPath.map(t=>(r=r&&void 0!==e.json[t],e.json[t]));r&&(n=s.get(o),t=s.keyPath.length)}}}if(n)return await new Promise((e,t)=>{n.onsuccess=(t=>e(t.target.result)),n.onerror=(e=>t(e.target.error))})}async deleteHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName),n=r.keyPath||e.idAttribute;return await new Promise((o,s)=>{t.oncomplete=(e=>o()),t.onerror=(e=>s(e.target.error)),r.delete(e.json[n]).onerror=(e=>s(e.target.error)),t.commit&&t.commit()})}async clearHandler(e){
const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName);return await new Promise((e,n)=>{t.oncomplete=(t=>e()),t.onerror=(e=>n(e.target.error)),r.clear().onerror=(e=>n(e.target.error)),t.commit&&t.commit()})}async queryHandler(e){const t=[];let r=0;const n=this.db.transaction([e.storeName],"readonly"),o=n.objectStore(e.storeName);let s,i,a;if(e.conditions){for(const d of o.indexNames)if(!s)if(a=o.index(d),e.conditions[a.keyPath]instanceof Array){const t=e.conditions[a.keyPath][0]>e.conditions[a.keyPath][1]?e.conditions[a.keyPath][1]:e.conditions[a.keyPath][0],r=e.conditions[a.keyPath][0]>e.conditions[a.keyPath][1]?e.conditions[a.keyPath][0]:e.conditions[a.keyPath][1];i=IDBKeyRange.bound(t,r,!0,!0),s=e.conditions[a.keyPath][0]>e.conditions[a.keyPath][1]?a.openCursor(i,IDBCursor.PREV||"prev"):a.openCursor(i,IDBCursor.NEXT||"next")
}else if("object"==typeof e.conditions[a.keyPath]&&("$gt"in e.conditions[a.keyPath]||"$gte"in e.conditions[a.keyPath]))i="$gt"in e.conditions[a.keyPath]?IDBKeyRange.lowerBound(e.conditions[a.keyPath].$gt,!0):IDBKeyRange.lowerBound(e.conditions[a.keyPath].$gte),s=a.openCursor(i,IDBCursor.NEXT||"next");else if("object"==typeof e.conditions[a.keyPath]&&("$lt"in e.conditions[a.keyPath]||"$lte"in e.conditions[a.keyPath])){let t;t="$lt"in e.conditions[a.keyPath]?IDBKeyRange.upperBound(e.conditions[a.keyPath].$lt,!0):IDBKeyRange.upperBound(e.conditions[a.keyPath].$lte),s=a.openCursor(t,IDBCursor.NEXT||"next")}else void 0!=e.conditions[a.keyPath]&&(i=IDBKeyRange.only(e.conditions[a.keyPath]),s=a.openCursor(i))}else if(e.index){a=o.index(e.index.name);const t=!!e.index.excludeLower,r=!!e.index.excludeUpper
;a&&(e.index.lower&&e.index.upper?i=IDBKeyRange.bound(e.index.lower,e.index.upper,t,r):e.index.lower?i=IDBKeyRange.lowerBound(e.index.lower,t):e.index.upper?i=IDBKeyRange.upperBound(e.index.upper,r):e.index.only&&(i=IDBKeyRange.only(e.index.only)),s="string"==typeof e.index.order&&"desc"===e.index.order.toLowerCase()?a.openCursor(i,IDBCursor.PREV||"prev"):a.openCursor(i,IDBCursor.NEXT||"next"))}else if(e.range){const t=e.range[0]>e.range[1]?e.range[1]:e.range[0],r=e.range[0]>e.range[1]?e.range[0]:e.range[1];i=IDBKeyRange.bound(t,r),s=e.range[0]>e.range[1]?o.openCursor(i,IDBCursor.PREV||"prev"):o.openCursor(i,IDBCursor.NEXT||"next")}else s=e.sort&&e.sort.index?-1===e.sort.order?o.index(e.sort.index).openCursor(null,IDBCursor.PREV||"prev"):o.index(e.sort.index).openCursor(null,IDBCursor.NEXT||"next"):o.openCursor();if(!s)throw new Error("No Cursor");return await new Promise((n,o)=>{s.onerror=(e=>o(e.target.error)),s.onsuccess=(o=>{const s=o.target.result
;s?e.limit&&t.length>=e.limit?i?e.conditions&&e.conditions[a.keyPath]?s.continue(e.conditions[a.keyPath][1]+1):e.index&&(e.index.upper||e.index.lower)&&("string"==typeof e.index.order&&"desc"===e.index.order.toLowerCase()?s.continue(e.index.lower):s.continue(e.index.upper)):s.continue():e.offset&&e.offset>r?(r++,s.continue()):(t.push(s.value),s.continue()):n(t)})})}async countHandler(e){const t=this.db.transaction([e.storeName],"readonly"),r=t.objectStore(e.storeName);return await new Promise((n,o)=>{let s;if(e.index){let t;if(!e.bound)throw new Error("Unsupported keyRange");t=IDBKeyRange.bound(e.bound.lower,e.bound.upper,e.bound.lowerOpen,e.bound.upperOpen);const n=r.index(e.index);s=n.count(t)}else s=r.count();s.onerror=(e=>o(e.target.error)),s.onsuccess=(e=>n(e.target.result)),t.commit&&t.commit()})}objectStoreNamesHandler(){return Array.from(this.db.objectStoreNames)}close(){console.error("XXX PORT"),this.db&&this.db.close()}}forsta.messenger.IDBGateway=class e{constructor(e){
this._rpc=e,this._rpc.addCommandHandler("db-gateway-init",this.onInitHandler.bind(this)),this._initialized=new Map}async onInitHandler(e){if(this._initialized.has(e.name))console.error("DB already initialized:",e.name);else{const r=new t(e.name,e.id,e.version,this._rpc);await r.init(),this._initialized.set(e.name,r)}}}}();