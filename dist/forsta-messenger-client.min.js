...
await this._rpc.invokeCommand("thread-set-attribute",e,t,r)}async threadArchive(e,t){await this._rpc.invokeCommand("thread-archive",e,t)}async threadRestore(e,t){await this._rpc.invokeCommand("thread-restore",e,t)}async threadExpunge(e,t){await this._rpc.invokeCommand("thread-expunge",e,t)}async threadDestroyMessages(e){await this._rpc.invokeCommand("thread-destroy-messages",e)}async threadSendMessage(e,t,r,n,s,o){return await this._rpc.invokeCommand("thread-send-message",e,t,r,n,s,o)}async threadSendUpdate(e,t,r){return await this._rpc.invokeCommand("thread-send-update",e,t,r)}}}(),function(){"use strict";self.forsta=self.forsta||{},forsta.messenger=forsta.messenger||{};const e={User:{migrations:[{version:1,migrate:function(e){const t=e.db.createObjectStore("messages");t.createIndex("threadId-received",["threadId","received"]),t.createIndex("threadId-read",["threadId","read"]),t.createIndex("sent","sent"),t.createIndex("expire","expire");const r=e.db.createObjectStore("receipts")
;r.createIndex("messageId","messageId");const n=e.db.createObjectStore("threads");n.createIndex("type-timestamp",["type","timestamp"]),e.db.createObjectStore("sessions"),e.db.createObjectStore("identityKeys"),e.db.createObjectStore("preKeys"),e.db.createObjectStore("signedPreKeys"),e.db.createObjectStore("state")}},{version:2,migrate:function(e){const t=e.objectStore("threads");t.createIndex("timestamp",["timestamp"])}},{version:3,migrate:function(e){const t=e.db.createObjectStore("cache");t.createIndex("bucket-expiration",["bucket","expiration"])}},{version:4,migrate:function(e){}},{version:5,migrate:function(e){}},{version:6,migrate:function(e){e.db.createObjectStore("contacts")}},{version:7,migrate:function(e){const t=e.objectStore("messages");t.createIndex("member","members",{multiEntry:!0})}},{version:8,migrate:function(e){const t=e.objectStore("threads");t.createIndex("pendingMember","pendingMembers",{multiEntry:!0})}},{version:9,migrate:function(e){}},{version:10,
migrate:function(e){const t=e.objectStore("messages");t.createIndex("ngrams3","ngrams3",{multiEntry:!0})}},{version:11,migrate:function(e){const t=e.db.createObjectStore("protocolReceipts");t.createIndex("sent","sent")}},{version:12,migrate:function(e){}},{version:13,migrate:function(e){}},{version:14,migrate:async function(e){const t=e.objectStore("threads");t.createIndex("archived-timestamp",["archived","timestamp"]),t.deleteIndex("type-timestamp");const r=t.openCursor();await new Promise((e,t)=>{r.onsuccess=(r=>{const n=r.target.result;if(n)try{void 0===n.value.archived&&n.update(Object.assign(n.value,{archived:0})),n.continue()}catch(e){throw t(e),e}else e()}),r.onerror=(e=>{console.error("cursor error event:",e),t(new Error("cursor error"))})})}},{version:15,migrate:function(e){const t=e.objectStore("messages");t.deleteIndex("ngrams3"),t.createIndex("from-ngrams","_from_ngrams",{multiEntry:!0}),t.createIndex("to-ngrams","_to_ngrams",{multiEntry:!0}),
t.createIndex("body-ngrams","_body_ngrams",{multiEntry:!0})}},{version:16,migrate:function(e){e.db.createObjectStore("trustedIdentities")}},{version:17,migrate:function(e){const t=e.db.createObjectStore("quarantinedMessages");t.createIndex("source","source")}},{version:18,migrate:function(e){const t=e.db.createObjectStore("counters");t.createIndex("model-fk-slot",["model","fk","slot"])}},{version:19,migrate:function(e){const t=e.objectStore("counters");t.createIndex("model-fk-slot-key",["model","fk","slot","key"])}},{version:20,migrate:function(e){const t=e.objectStore("messages");t.createIndex("threadId-serverReceived",["threadId","serverReceived"]),t.deleteIndex("threadId-received")}},{version:21,migrate:function(e){const t=e.objectStore("messages");t.createIndex("messageRef","messageRef")}},{version:22,migrate:async function(e){const t=e.objectStore("messages");t.createIndex("threadId-timestamp",["threadId","timestamp"]),t.deleteIndex("threadId-serverReceived")
;const r=t.openCursor();await new Promise((e,t)=>{r.onsuccess=(r=>{const n=r.target.result;if(n)try{const e=n.value.serverReceived||n.value.sent,r=Object.assign({},n.value,{timestamp:e});delete r.serverReceived,n.update(r),n.continue()}catch(e){throw t(e),e}else e()}),r.onerror=(e=>{console.error("cursor error event:",e),t(new Error("cursor error"))})})}}]},SharedCache:{migrations:[{version:1,migrate:function(e){const t=e.db.createObjectStore("cache");t.createIndex("bucket-expiration",["bucket","expiration"])}}]}};class t{constructor(t,r,n,s){if(this.schema=e[t],this.id=r,this.version=n,this._rpc=s,!this.schema)throw new Error("No Database Schema")}async init(){console.info(`Opening database ${this.id} (v${this.version})`);const e=indexedDB.open(this.id,this.version);await new Promise((t,r)=>{e.onblocked=(e=>{this._rpc.triggerEvent("db-gateway-blocked",this.id)}),e.onsuccess=(e=>{const r=this.db=e.target.result;r.onversionchange=(e=>{
console.warn("Database version change requested somewhere: Closing our connection!");try{r.close()}finally{this._rpc.triggerEvent("db-gateway-versionchange",this.id)}}),t()}),e.onerror=(e=>{r(new Error("Could not connect to the database"))}),e.onabort=(e=>{r(new Error("Connection to the database aborted"))}),e.onupgradeneeded=(async t=>{console.warn(`Database upgrade needed: v${t.oldVersion} => v${t.newVersion}`),this.db=t.target.result;try{await this.migrate(e.transaction,t.oldVersion,t.newVersion)}catch(e){r(e)}})}),this._rpc.addCommandHandler(`db-gateway-read-${this.id}`,this.readHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-update-${this.id}`,this.updateHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-query-${this.id}`,this.queryHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-delete-${this.id}`,this.deleteHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-clear-${this.id}`,this.clearHandler.bind(this)),
this._rpc.addCommandHandler(`db-gateway-create-${this.id}`,this.createHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-count-${this.id}`,this.countHandler.bind(this)),this._rpc.addCommandHandler(`db-gateway-object-store-names-${this.id}`,this.objectStoreNamesHandler.bind(this))}async migrate(e,t,r){console.info(`DB migrate begin version from v${t} to ${r}`),e.onerror=(e=>{throw new Error("Unhandled migration error:",e)}),e.onabort=(e=>{throw new Error("Unhandled migration abort:",e)});const n=Array.from(this.schema.migrations).filter(e=>e.version>t&&e.version<=r);if(!n||n[n.length-1].version!==r)throw console.error(`Missing migrations for: v${r}; `+`available: ${n.map(e=>e.version)}`),new Error(`Missing migrations for target version: ${r}`);for(const s of n)console.warn("DB migrating to:",s.version),await s.migrate(e),console.info("DB migrated successfully:",s.version)}async createHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName)
;return e.idFallback&&!r.autoIncrement&&(e.json[e.idAttribute]=e.idFallback),await new Promise((n,s)=>{let o;return t.oncomplete=(e=>n()),t.onerror=(e=>s(e.target.error)),(o=r.keyPath?r.add(e.json):r.add(e.json,e.json[e.idAttribute])).onerror=(e=>s(e.target.error)),t.commit&&t.commit(),e.json})}async updateHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName);return await new Promise((n,s)=>{let o;t.oncomplete=(e=>n()),t.onerror=(e=>s(e.target.error)),(o=r.keyPath?r.put(e.json):r.put(e.json,e.json[e.idAttribute])).onerror=(e=>s(e.target.error)),t.commit&&t.commit()})}async readHandler(e){const t=this.db.transaction([e.storeName],"readonly"),r=t.objectStore(e.storeName);let n;if(e.json[e.idAttribute])n=r.get(e.json[e.idAttribute]);else if(e.index){const t=r.index(e.index.name);n=t.get(e.index.value)}else{let t=0;for(const s of r.indexNames){const o=r.index(s)
;if("string"==typeof o.keyPath&&1>t)void 0!==e.json[o.keyPath]&&(n=o.get(e.json[o.keyPath]),t=1);else if("object"==typeof o.keyPath&&o.keyPath.length>t){let r=!0;const s=o.keyPath.map(t=>(r=r&&void 0!==e.json[t],e.json[t]));r&&(n=o.get(s),t=o.keyPath.length)}}}if(n)return await new Promise((e,t)=>{n.onsuccess=(t=>e(t.target.result)),n.onerror=(e=>t(e.target.error))})}async deleteHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName),n=r.keyPath||e.idAttribute;return await new Promise((s,o)=>{t.oncomplete=(e=>s()),t.onerror=(e=>o(e.target.error)),r.delete(e.json[n]).onerror=(e=>o(e.target.error)),t.commit&&t.commit()})}async clearHandler(e){const t=this.db.transaction([e.storeName],"readwrite"),r=t.objectStore(e.storeName);return await new Promise((e,n)=>{t.oncomplete=(t=>e()),t.onerror=(e=>n(e.target.error)),r.clear().onerror=(e=>n(e.target.error)),t.commit&&t.commit()})}async queryHandler(e){const t=[];let r=0
;const n=this.db.transaction([e.storeName],"readonly"),s=n.objectStore(e.storeName);let o,a,i;if(e.conditions){for(const d of s.indexNames)if(!o)if(i=s.index(d),e.conditions[i.keyPath]instanceof Array){const t=e.conditions[i.keyPath][0]>e.conditions[i.keyPath][1]?e.conditions[i.keyPath][1]:e.conditions[i.keyPath][0],r=e.conditions[i.keyPath][0]>e.conditions[i.keyPath][1]?e.conditions[i.keyPath][0]:e.conditions[i.keyPath][1];a=IDBKeyRange.bound(t,r,!0,!0),o=e.conditions[i.keyPath][0]>e.conditions[i.keyPath][1]?i.openCursor(a,IDBCursor.PREV||"prev"):i.openCursor(a,IDBCursor.NEXT||"next")}else if("object"==typeof e.conditions[i.keyPath]&&("$gt"in e.conditions[i.keyPath]||"$gte"in e.conditions[i.keyPath]))a="$gt"in e.conditions[i.keyPath]?IDBKeyRange.lowerBound(e.conditions[i.keyPath].$gt,!0):IDBKeyRange.lowerBound(e.conditions[i.keyPath].$gte),
o=i.openCursor(a,IDBCursor.NEXT||"next");else if("object"==typeof e.conditions[i.keyPath]&&("$lt"in e.conditions[i.keyPath]||"$lte"in e.conditions[i.keyPath])){let t;t="$lt"in e.conditions[i.keyPath]?IDBKeyRange.upperBound(e.conditions[i.keyPath].$lt,!0):IDBKeyRange.upperBound(e.conditions[i.keyPath].$lte),o=i.openCursor(t,IDBCursor.NEXT||"next")}else void 0!=e.conditions[i.keyPath]&&(a=IDBKeyRange.only(e.conditions[i.keyPath]),o=i.openCursor(a))}else if(e.index){i=s.index(e.index.name);const t=!!e.index.excludeLower,r=!!e.index.excludeUpper;i&&(e.index.lower&&e.index.upper?a=IDBKeyRange.bound(e.index.lower,e.index.upper,t,r):e.index.lower?a=IDBKeyRange.lowerBound(e.index.lower,t):e.index.upper?a=IDBKeyRange.upperBound(e.index.upper,r):e.index.only&&(a=IDBKeyRange.only(e.index.only)),o="string"==typeof e.index.order&&"desc"===e.index.order.toLowerCase()?i.openCursor(a,IDBCursor.PREV||"prev"):i.openCursor(a,IDBCursor.NEXT||"next"))}else if(e.range){
const t=e.range[0]>e.range[1]?e.range[1]:e.range[0],r=e.range[0]>e.range[1]?e.range[0]:e.range[1];a=IDBKeyRange.bound(t,r),o=e.range[0]>e.range[1]?s.openCursor(a,IDBCursor.PREV||"prev"):s.openCursor(a,IDBCursor.NEXT||"next")}else o=e.sort&&e.sort.index?-1===e.sort.order?s.index(e.sort.index).openCursor(null,IDBCursor.PREV||"prev"):s.index(e.sort.index).openCursor(null,IDBCursor.NEXT||"next"):s.openCursor();if(!o)throw new Error("No Cursor");return await new Promise((n,s)=>{o.onerror=(e=>s(e.target.error)),o.onsuccess=(s=>{const o=s.target.result;o?e.limit&&t.length>=e.limit?a?e.conditions&&e.conditions[i.keyPath]?o.continue(e.conditions[i.keyPath][1]+1):e.index&&(e.index.upper||e.index.lower)&&("string"==typeof e.index.order&&"desc"===e.index.order.toLowerCase()?o.continue(e.index.lower):o.continue(e.index.upper)):o.continue():e.offset&&e.offset>r?(r++,o.continue()):(t.push(o.value),o.continue()):n(t)})})}async countHandler(e){
const t=this.db.transaction([e.storeName],"readonly"),r=t.objectStore(e.storeName);return await new Promise((n,s)=>{let o;if(e.index){let t;if(!e.bound)throw new Error("Unsupported keyRange");t=IDBKeyRange.bound(e.bound.lower,e.bound.upper,e.bound.lowerOpen,e.bound.upperOpen);const n=r.index(e.index);o=n.count(t)}else o=r.count();o.onerror=(e=>s(e.target.error)),o.onsuccess=(e=>n(e.target.result)),t.commit&&t.commit()})}objectStoreNamesHandler(){return Array.from(this.db.objectStoreNames)}}forsta.messenger.IDBGateway=class e{constructor(e){this._rpc=e,this._rpc.addCommandHandler("db-gateway-init",this.onInitHandler.bind(this)),this._initialized=new Map}async onInitHandler(e){if(!this._initialized.has(e.name)){const r=new t(e.name,e.id,e.version,this._rpc);await r.init(),this._initialized.set(e.name,r)}}}}();